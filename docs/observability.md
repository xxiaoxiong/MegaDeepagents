...
服务端每 200ms 检查一次新增事件，并每 15 秒发送一次 keepalive；在不引入额外消息代理的
前提下，把 SQLite 事件进入浏览器的轮询延迟控制在约 200ms。前端只有在历史游标初始化完成
后才订阅实时流，且每收到一个事件都会推进重连游标，避免历史与直播竞态或断线后重复回放。

模型回调会把提供商返回的最终 Assistant 文本作为权威内容写入终态事件；即使模型报错，
也会先刷新已经收到的部分 token，再写入带错误信息的终态消息。每次工具开始事件同样保证
对应一个成功或失败的结束事件，包括参数校验失败、提前返回和异常路径，避免界面长期停留
在“执行中”。工具事件的耗时只覆盖真实工具执行，模型决定是否调用工具的时间应在 LLM
trace 中观察。

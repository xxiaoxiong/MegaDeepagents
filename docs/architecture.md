...
调度器每轮只创建不超过剩余并发槽、且能匹配到不同空闲 Agent 的 Future。匹配优先处理
候选最少的 Task，并优先选择能力范围更窄的 Worker，给真正需要通才的任务保留通才。
这样不会为同一个空闲 Agent 同时创建大量必然认领失败的 Future，也不会用空转轮次消耗
执行预算；工具类能力的兼容回退与 AgentRegistry 使用同一语义。

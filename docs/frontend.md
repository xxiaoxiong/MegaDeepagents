...
对话页会先取得历史游标，再建立 SSE，避免迟到的历史响应覆盖实时消息。实时事件到达后，
`after_sequence` 会同步前移；Assistant 消息始终通过 Vue 响应式代理增量修改，因此 token
会在同一个气泡中持续增长，结束、失败或断线时也会及时关闭流式光标。

工具调用默认只显示一行语义摘要、状态和耗时。点击后优先展示返回结果；参数收进二级折叠区。
`read_file` 一类读取工具会通过受 Run 边界保护的文件 API 分块加载完整内容，而不是只展示
事件里的 2,000 字符预览。
这里的耗时从工具真正开始执行时计时，不包含模型在调用工具之前的推理时间。
仓库合并后的测试/构建门禁会显示开始、完成、耗时和环境阻塞；缺少运行时或依赖时使用警告
状态并进入可恢复人工处理，不会把环境问题显示成 Agent 已完成。
...
聊天里的 Artifact 以紧凑文件名按钮呈现，不再把 opaque ID 当作主标题。右侧 Explorer
默认打开用户点击的文件，使用单列文件选择器与可滚动全高预览；大文本按游标分块读取直到
完整。它同时支持搜索、Markdown/文本/常见图片预览、版本 lineage、复制链接/内容和下载。
`#artifact-{artifact_id}` 深链接会在刷新后恢复选择并滚动到产物区域。
